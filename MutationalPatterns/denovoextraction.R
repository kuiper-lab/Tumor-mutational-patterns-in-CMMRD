...
sbs_mutation_matrix_combined <- mut_mat_all + 0.0001
if (perform_estimate){
  print("Starting estimation")
  estimate <- NMF::nmf(sbs_mutation_matrix_combined, rank=2:15, method="brunet", nrun=10, seed=123456)
...
if (perform_NMF){
  print("Starting NMF")
  nmf_res_sbs_mutation_matrix_combined <- extract_signatures(sbs_mutation_matrix_combined,
                                                             rank = number_clusters,
                                                             nrun = 200,
							     single_core = TRUE)
  save(nmf_res_sbs_mutation_matrix_combined,
     file = nmf_location)
}
